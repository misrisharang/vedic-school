...
// Helper to generate reliable official Unicode flag emoji from 2-letter ISO country code
...
// Countries where students are currently learning with Meenakshi Koul worldwide (USA and Canada excluded)
...
/**
 * Normalizes user-typed phone input into clean digits,
 * stripping accidental dial code prefixes and leading trunk 0s.
 */
export const cleanLocalDigits = (phone: string, dialCode: string): string => {
  // Strip all non-digits
  let digits = phone.replace(/\D/g, '');

  // Extract dial code digits (e.g. "+91" -> "91", "+1" -> "1")
  const dialDigits = dialCode.replace(/\D/g, '');

  // If user accidentally typed dial code first (e.g. "919876543210" when dialCode is "+91")
  // and there are plenty of subsequent digits, strip the redundant dial code prefix
  if (dialDigits && digits.startsWith(dialDigits) && digits.length >= dialDigits.length + 7) {
    digits = digits.slice(dialDigits.length);
  }

  // Strip leading zero commonly used in local dialling (e.g. "050..." or "07...")
  // Only if digits length is 9 or more
...
/**
 * Normalizes full WhatsApp number into complete E.164 international format (e.g. "+919876543210").
 */
export const normalizeWhatsAppNumber = (phone: string, dialCode: string): string => {
  const digits = cleanLocalDigits(phone, dialCode);
  const cleanDialCode = dialCode.startsWith('+') ? dialCode : `+${dialCode}`;
  return `${cleanDialCode}${digits}`;
};

/**
 * Validates phone number according to country rules and international standards.
 */
export const validatePhoneNumber = (
  ...
  // Check for invalid characters (only digits, spaces, hyphens, plus, parentheses allowed)
  if (!/^[\d\s\-()+]+$/.test(trimmed)) {
    return { isValid: false, error: 'Please enter a valid phone number containing digits.' };
  }

  const country = getCountryByCode(countryCode);
  const digits = cleanLocalDigits(trimmed, country.dialCode);
  ...
  switch (countryCode) {
    ...
    case 'GB': // UK: 10 or 11 digits (mobile is 10 digits after stripping trunk 0)
      if (digits.length < 9 || digits.length > 11) {
        return {
          isValid: false,
          error: 'Please enter a valid UK phone number (10 digits).',
        };
      }
      break;

    case 'AE': // UAE: 9 digits (typically mobile 5x xxx xxxx)
      if (digits.length !== 9) {
        return {
          isValid: false,
          error: 'Please enter a valid 9-digit UAE phone number (e.g. 50 123 4567).',
        };
      }
      break;

    case 'AU': // Australia: 9 digits (mobile 4xx xxx xxx)
    ...
    default:
      // International standard ITU-T E.164 allows 6 to 15 digits
